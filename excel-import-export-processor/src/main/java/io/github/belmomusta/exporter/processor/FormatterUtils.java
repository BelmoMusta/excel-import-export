...
public class FormatterUtils {
    ...
    static void lookForFormatters(ClassWrapper classWrapper) {
        Element formatAnnotation =
                ExportProcessor.processingEnvironment.getElementUtils().getTypeElement(ColumnFormat.class.getName());
        for (MethodWrapper method : classWrapper.getEnclosedMethods()) {
            boolean formatterFound = tryToFindFormatters(formatAnnotation, method,
                    method.wrappedElement.getAnnotationMirrors());
            ...
            if (!formatterFound) {
                for (FieldWrapper enclosedField : classWrapper.getEnclosedFields()) {
                    ...
                    if (enclosedField.getName().equals(possibleFieldName)) {
                        tryToFindFormatters(formatAnnotation, method,
                                enclosedField.wrappedElement.getAnnotationMirrors());
                    }
                }
            }
        }
